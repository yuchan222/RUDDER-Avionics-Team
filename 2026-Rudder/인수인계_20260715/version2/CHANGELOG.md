...
- `checkLanded()`에서 BMP 실패(`altitude_cm == -1`) 시 착지로 오판하는 문제 발견 및 수정
- 사출 후 BMP 센서가 계속 실패값을 반환하면 "고도 변화 없음"으로 오인식되어  
  실제 착지 전에 Mode 4로 자동 전환될 위험이 있었음
- `p.altitude_cm == -1` 가드 추가로 해결 (`EjectionLogic.cpp`)
