...
static bool       g_baselineOk  = true;   // 마지막 ARMED 진입 시 기준압 수집 성공 여부 (GCS 표시용)
...
static void checkLaunch(uint32_t now) {
  ...
  bool bmpOk = (g_pkt.altitude_cm != ALT_INVALID);
...
void setup() {
  ...
  Serial.print("[BOOT] version5  mode=0 대기  status=0x");
