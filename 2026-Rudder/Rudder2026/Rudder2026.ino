// ═══════════════════════════════════════════════════════════════════════════
//  Rudder2026 — RUDDER 팀 비행 소프트웨어 (자세추정 + 3D 시각화, 구 version8)
//
...
#define CMD_SYSRESET     0x63   // 소프트 리셋 (대기·착륙 모드에서만)
...
static void handleCommand(uint8_t cmd, uint32_t now) {
  ...
  switch (cmd) {
    ...
    case CMD_FORCE_LAND:
      ...
      break;
    case CMD_SYSRESET:
      // 대기(SAFE) 상태뿐 아니라 착륙(LANDED) 상태에서도 허용 — 테스트 종료 후
      // 뚜껑 안 열고 GCS에서 리셋하는 용도. 비행 중(준비·비행·낙하)엔 여전히 차단.
      if (g_mode == MODE_SAFE || g_mode == MODE_LANDED) NVIC_SystemReset();
      break;
    default:
...
void setup() {
  ...
  Serial.print("[BOOT] Rudder2026  mode=0 대기  status=0x");
