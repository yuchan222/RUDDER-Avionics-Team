...
// ── I2C 버스 락업 복구 (2026-08-06 추가) ──────────────────────────────────
// BMP388·IMU·INA219이 전부 동시에 응답 없어지는 증상 확인 — 개별 칩 3개가
// 우연히 동시고장난 게 아니라, 이 셋이 공유하는 I2C 버스 자체가 멈춘(lockup)
// 것으로 판단됨. 슬레이브 하나가 SDA를 붙잡은 채 멈추면 버스 전체가 막히고,
// 이건 MCU만 재시작(소프트 리셋)해도 안 풀림 — 실제로 전원을 완전히 껐다
// 켜야만 복구되던 걸 확인함. 표준 절차(SCL을 최대 9번 토글해 슬레이브가
// SDA를 놓게 강제)로 소프트웨어에서도 복구를 시도해봄.
static void i2cBusClear() {
  ...
  for (int i = 0; i < 9; i++) {
    ...
    delayMicroseconds(5);
    if (digitalRead(SDA) == HIGH) break;   // 슬레이브가 SDA를 놓으면 더 안 해도 됨
  }
  // STOP 조건 흉내 (SCL이 HIGH인 상태에서 SDA를 LOW->HIGH)
  pinMode(SDA, OUTPUT);
...
// ── 공개 함수 ─────────────────────────────────────────────────────────────
void initSensors() {
  i2cBusClear();   // Wire.begin() 전에 먼저 — 이전 세션에서 버스가 멈춰있었을 경우 대비
  Wire.begin();
  ...
  if (imuRead(acc, gyro)) {
    ...
    if (allZero) {
      if (++s_imuZeroCount >= IMU_ZERO_RESET_COUNT) {
        Serial.println("[IMU] 0,0,0 고착 감지 — I2C 버스 클리어 + 재초기화 시도");
        // 단순 재초기화만으론 버스 락업(BMP/INA까지 같이 죽는 유형)은 못 풀림 —
        // 버스 클리어를 먼저 해서 BMP/INA도 이후 자기 읽기 루프에서 같이 살아나게 함
        i2cBusClear();
        Wire.begin();
        Wire.setClock(100000);
        imuInit();
    ...
    if (dt > 0.0f && dt < 0.5f) {   // 첫 호출이나 비정상적으로 큰 dt는 건너뜀 (튐 방지)
      ...
      // 지속 회전(텀블링) 시 자이로 적분값이 ±180°를 넘어 무한히 누적되면
      // accAngleXY/XZ(항상 -180~180로 정규화됨)와 블렌딩할 때 튀거나, int16_t
      // 전송범위(±327.67°)를 넘어 오버플로될 수 있어 매 샘플마다 감싸줌.
      while (s_tiltXY > 180.0f)  s_tiltXY -= 360.0f;
      while (s_tiltXY < -180.0f) s_tiltXY += 360.0f;
      while (s_tiltXZ > 180.0f)  s_tiltXZ -= 360.0f;
      while (s_tiltXZ < -180.0f) s_tiltXZ += 360.0f;
    }
    ...
    s_imuOk = false;
    // I2C 응답 자체가 없는 경우(버스 락업 의심 — SD 로그로 실제 확인된 증상:
    // BMP·IMU·INA 동시에 응답 없어지고 소프트 리셋으론 안 풀림) — 0,0,0 고착과는
    // 별개 경로라 여기서도 똑같이 카운트해서 버스 클리어를 시도함
    if (++s_imuZeroCount >= IMU_ZERO_RESET_COUNT) {
      Serial.println("[IMU] I2C 무응답 지속 — I2C 버스 클리어 + 재초기화 시도");
      i2cBusClear();
      Wire.begin();
      Wire.setClock(100000);
      imuInit();
