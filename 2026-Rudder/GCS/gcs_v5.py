"""
RUDDER 2026 — GCS version5 (version5 펌웨어 전용, 기존 gcs_v4.py는 보존)

gcs_v4.py에서 검증된 구조를 유지하면서 2026-07-29 GPT 기술검토 대응으로
다음을 추가/변경 (자세한 내용은 세션 기록의 "오늘 수정 정리" 참고):

  1. 모드 5단계 새 의미: 0 대기 / 1 준비 / 2 비행(발사감지됨) / 3 낙하 / 4 착륙
  2. 모드 전환 버튼은 정해진 출발 모드에서만 활성화 + 비상사출/강제비행 2단계 확인
  3. 비상사출: eject_state 기반 확인 + 30초 재전송 시간제한 + 재연결 시 상태 자동초기화
  4. 텔레메트리 CRC16 검증, 서보 전원 ON/OFF 표시(MOSFET 신호 기준)
  5. 재연결 시 그래프 완전히 새로 그리기 (이전 세션 잔상 제거)
  6. 기준압 수집 실패 시 GCS에 경고 표시("기준압" 상태점, 진입 자체는 차단 안 함)
  7. altitude_cm 무효값 판정 기준을 ALT_INVALID(INT32_MIN)로 변경 (펌웨어와 동기화)
  8. 깜빡임 최소화: 전체 rerun 없음, 수치는 0.3초/그래프는 1초 부분 갱신

실행:  python -m streamlit run gcs_v5.py
"""
...
# ─── 상수 (version5 Config.h / Packet.h와 동기화) ────────────────────────────
...
def open_log():
    ...
    path = LOG_DIR / f"gcs_v5_{datetime.now().strftime('%Y%m%d_%H%M%S')}.csv"
...
# ─── Streamlit ───────────────────────────────────────────────────────────────
st.set_page_config(page_title="RUDDER GCS v5", layout="wide", page_icon="🚀")
...
def _sidebar():
    with st.sidebar:
        ...
        if st.button("연결", type="primary", use_container_width=True) and devs:
            ...
            if old:
                old.stop()
            link = SikLink(devs[disp.index(sel)])
            try:
                link.start()
...
def _charts():
    ...
    fig.add_trace(go.Scatter(x=ts, y=st.session_state.acc_hist, mode='lines',
                  ...
                  row=2, col=1)
    fig.add_hline(y=1000, line_dash='dash', line_color='#888', row=2, col=1)
