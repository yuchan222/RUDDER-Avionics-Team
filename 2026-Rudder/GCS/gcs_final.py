...
PKT_FMT    = '<HBHBHIhhhhhhihihhBBBHBHH'   # 48바이트 — version6 전용
...
class RocketGCS(tk.Tk):
    ...
    def _build_ui(self):
        ...
        main.columnconfigure(0, weight=1)   # 왼쪽: 그래프 2개 (거의 정사각형)
        main.columnconfigure(1, weight=1)   # 오른쪽: 나머지 전부
    ...
    # ── 왼쪽: 그래프 2개, 세로로 반반 (전체 높이를 다 써서 정사각형에 가깝게) ──
    def _build_graphs(self, parent):
        ...
        alt_panel.grid(row=0, column=0, sticky="nsew", pady=(0, 3))
    ...
    def _build_right(self, parent):
        ...
        parent.rowconfigure(4, weight=1)   # 이벤트 로그 (남는 공간 다 차지)
    ...
    def _build_datacards(self, parent):
        ...
        row.columnconfigure(list(range(6)), weight=1, uniform="card")
    ...
    def _build_event_log(self, parent):
        ...
        남는 세로 공간을 이 패널이 다 가져가도록 weight=1로 배치됨."""
        log_panel = self._panel(parent, "이벤트 로그", MUTED)
        log_panel.grid(row=4, column=0, sticky="nsew", pady=(0, 6))
