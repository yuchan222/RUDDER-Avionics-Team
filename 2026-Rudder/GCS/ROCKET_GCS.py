"""
ROCKET_GCS — RUDDER 2026 지상국 (자세추정 + 3D 로켓 뷰, 구 gcs_v8.py)

gcs_final.py 기반. Rudder2026 펌웨어(구 version8, 52바이트 패킷, tilt_xy/tilt_xz
포함) 전용 — version_final 이하와는 패킷 포맷이 달라 안 맞음.
pywebview가 설치돼 있으면 실제 Fusion360 OBJ 로켓 모델이 회전하는 3D 창을
별도로 띄움(assets/attitude3d.html, Edge WebView2 사용). 설치 안 돼있거나
실패하면 3D 창 없이 Tkinter만 정상 실행됨 (완전히 선택적 기능).

실행: python ROCKET_GCS.py   (Streamlit 아님, 그냥 python으로 실행)
"""
...
_webview_window = None   # webview.start() 이후 실제 Window 객체로 채워짐
...
# ═══════════════════════════════════════════════════════════════════════════
#  통신/패킷 계층 (gcs_v6.py 기반 — Rudder2026 펌웨어 52바이트 패킷, tilt_xy/tilt_xz 추가)
# ═══════════════════════════════════════════════════════════════════════════
...
CMD_FORCE_LAND   = 0x51
CMD_SYSRESET     = 0x63   # 소프트 리셋 — 펌웨어가 대기(SAFE) 모드에서만 받아줌
...
class FakeLink:
    ...
    def send_cmd(self, cmd: int):
        ...
        if cmd == CMD_SET_ARMED and self._mode == 0:
            ...
            self._mode = 4
        elif cmd == CMD_SYSRESET and self._mode in (0, 4):
            # 실제 펌웨어의 NVIC_SystemReset() 재부팅을 모사 — ms가 0 근처로
            # 되돌아가서 GCS의 "보드 재시작 감지" 로직을 테스트할 수 있게 함
            self._t0 = time.time()
            self._seq = 0
            self._cmd_rx = 0
            self._last_cmd = 0
...
LOG_FIELDS = ['rx_time', 'seq', 'ms', 'flight_mode', 'eject_state', 'altitude_cm',
              ...
              'system_status', 'cmd_rx_count', 'last_cmd', 'tilt_xy', 'tilt_xz']


def open_log():
    LOG_DIR.mkdir(exist_ok=True)
    path = LOG_DIR / f"ROCKET_GCS_{datetime.now().strftime('%Y%m%d_%H%M%S')}.csv"
...
class Sparkline(tk.Canvas):
    ...
    def update_data(self, data, times=None, peak=None):
        self.delete("all")
        ...
        # 최고고도 (상승 중엔 실시간 갱신, 하강 시작하면 그 값으로 고정 — GCS가
        # 수신 고도의 running max를 추적해서 넘겨줌. 현재값 바로 아래에 표시)
        if peak is not None:
            self.create_text(w - pad_r, 24, text=f"최고 {self._fmt.format(peak)}{self._unit}",
                              font=("Courier", 11, "bold"), fill=AMBER, anchor="ne")
...
class RocketGCS(tk.Tk):
    ...
    def __init__(self):
        super().__init__()
        self.title("ROCKET_GCS — Rudder2026 (자세추정 실험)")
        ...
        self.landed_ref_ms = None      # 착륙 진입 시점 ms — 서보 전원 유지시간(SERVO_CLOSE_HOLD_MS) 계산용
        self.graph_frozen = False      # 착륙하면 그래프 그대로 고정(더 안 밀려나게)
        self.peak_alt_m = None         # 수신 고도의 running max — 하강 시작하면 더 안 늘어나서 자동으로 정점에 고정됨
        ...
        if _HAS_WEBVIEW and _HAS_WIN32:
            self.after(150, self._try_embed_3d)
        self._tick()
    ...
    def _build_ui(self):
        ...
        main.columnconfigure(0, weight=2)   # 왼쪽: 2x2 정사각형 그리드 (고도/가속도/자세/이벤트로그)
        main.columnconfigure(1, weight=3)   # 오른쪽: 나머지(데이터카드/센서/서보모드/타임라인/비상사출) — 더 넓게
    ...
    # ── pywebview 3D 창을 attitude_container 프레임 안으로 재부모화 ─────────
    def _try_embed_3d(self, _attempt=0):
        """webview.start()가 만드는 네이티브 창은 별도 스레드/시점에 생성되므로,
        제목으로 찾일 때까지 짧게 재시도. 찾으면 win32 SetParent로 자식 창으로
        바꿔서 GCS 메인 윈도우의 '로켓 자세' 칸 안에 실제로 끼워넣음.
        (재시도 횟수를 제한 — 창이 끝내 안 뜨면(생성 실패/닫힘) after()가
        무한히 쌓이는 것을 방지)"""
        hwnd = win32gui.FindWindow(None, "로켓 자세 3D")
        if not hwnd:
            if _attempt < 40:   # 200ms * 40 = 약 8초까지만 재시도
                self.after(200, lambda: self._try_embed_3d(_attempt + 1))
            return
        try:
            style = win32gui.GetWindowLong(hwnd, win32con.GWL_STYLE)
            style &= ~(win32con.WS_CAPTION | win32con.WS_THICKFRAME | win32con.WS_SYSMENU |
                       win32con.WS_MINIMIZEBOX | win32con.WS_MAXIMIZEBOX | win32con.WS_POPUP)
            style |= win32con.WS_CHILD
            win32gui.SetWindowLong(hwnd, win32con.GWL_STYLE, style)
            win32gui.SetParent(hwnd, self.attitude_container.winfo_id())
            # 스타일만 바꾸면 화면이 안 갱신될 수 있어 SWP_FRAMECHANGED로 강제 재계산
            ...
            print(f"[3D] 창 임베드 실패 — 별도 창으로 유지됨: {e}")

    def _reposition_3d(self, event=None):
        if not self._embedded_hwnd:
            return
        w = self.attitude_container.winfo_width()
        h = self.attitude_container.winfo_height()
        if w < 2 or h < 2:
            return
        try:
    ...
    def _build_topbar(self):
        ...
        # 아두이노 뚜껑 안 열고 리셋할 수 있게 — 펌웨어가 대기(SAFE)·착륙(LANDED)
        # 모드에서만 받아주므로 그 외 모드에선 버튼 자체를 비활성화 (_tick에서 갱신)
        self.reset_btn = tk.Button(
            bar, text="🔄 리셋", font=("Courier", 9, "bold"), bg=SURFACE2, fg=MUTED,
            bd=0, activebackground=BORDER, cursor="hand2", padx=8, pady=3,
            highlightthickness=1, highlightbackground=BORDER2, state="disabled",
            command=self._send_reset)
        self.reset_btn.pack(side="right", padx=(4, 0), pady=6)
    ...
    def _send_reset(self):
        if not self.link:
            return
        self.link.send_cmd(CMD_SYSRESET)
        self._log_event("시스템 리셋 명령 전송 — 보드 재부팅 대기", color=AMBER)
    ...
    def _build_grid(self, parent):
        ...
        att_panel = self._panel(parent, "로켓 자세 (3D)", CYAN)
        att_panel.grid(row=1, column=0, sticky="nsew", padx=(0, 3), pady=(3, 0))
        # pywebview 3D 창(attitude3d.html)을 이 프레임 안에 실제로 끼워넣는(embed)
        # 컨테이너 — win32 SetParent로 재부모화됨. _try_embed_3d() 참고.
        self.attitude_container = tk.Frame(att_panel, bg="#0d1117")
        self.attitude_container.pack(fill="both", expand=True, padx=4, pady=4)
        self.attitude_container.bind("<Configure>", self._reposition_3d)
    ...
    def _build_right(self, parent):
        ...
        parent.rowconfigure(0, weight=1)   # 데이터 카드 — 남는 공간 나눠 받아서 커짐
        parent.rowconfigure(1, weight=1)   # 센서 상태
        parent.rowconfigure(2, weight=1)   # 모드제어
        parent.rowconfigure(3, weight=1)   # 타임라인 — 이제 혼자 여백을 다 떠안지 않음
    ...
    def _build_datacards(self, parent):
        ...
        row.grid(row=0, column=0, sticky="nsew", pady=(0, 6))
        row.columnconfigure((0, 1), weight=1, uniform="card")
        row.rowconfigure((0, 1, 2), weight=1, uniform="card")
    ...
    def _build_sensor_panel(self, parent):
        """센서 5개 + 서보 전원 상태를 합쳐 3열×2행, 칸을 꽉 채우는 큰 박스로 배치
        (예전엔 작은 배지가 왼쪽에 몰려있고 나머지는 빈 여백이었음)."""
        sens_panel = self._panel(parent, "센서 상태", MUTED)
        sens_panel.grid(row=1, column=0, sticky="nsew", pady=(0, 6))
        sens_grid = tk.Frame(sens_panel, bg=SURFACE)
        sens_grid.pack(fill="both", expand=True, padx=8, pady=8)
        sens_grid.columnconfigure((0, 1, 2), weight=1, uniform="sens")
        sens_grid.rowconfigure((0, 1), weight=1, uniform="sens")

        self.sensor_frames = {}
        names = list(self.SENSOR_NAMES) + ["서보"]
        for i, name in enumerate(names):
            f = tk.Frame(sens_grid, bg=RED_DIM, bd=0, highlightthickness=1, highlightbackground=RED)
            f.grid(row=i // 3, column=i % 3, sticky="nsew", padx=3, pady=3)
            inner = tk.Frame(f, bg=RED_DIM)
            inner.pack(expand=True)
            dot = tk.Label(inner, text="●", font=("Arial", 14), bg=RED_DIM, fg=RED)
            dot.pack(side="left", padx=(0, 6))
            lbl = tk.Label(inner, text=name, font=("Courier", 13, "bold"), bg=RED_DIM, fg=RED)
            lbl.pack(side="left")
            self.sensor_frames[name] = (f, inner, dot, lbl)

    def _build_ctrl_row(self, parent):
        """서보 전원 상태는 센서 상태 패널의 6번째 박스로 옮겨서(_build_sensor_panel),
        여기엔 모드 제어만 남기고 전체 너비로 확장 — 위 센서 패널 여백을 줄이면서
        여기도 같이 세로로 늘어나 타임라인 쪽 여백을 나눠 가짐."""
        ctrl_row = tk.Frame(parent, bg=BG)
        ctrl_row.grid(row=2, column=0, sticky="nsew", pady=(0, 6))
        ctrl_row.columnconfigure(0, weight=1)
        ctrl_row.rowconfigure(0, weight=1)

        mode_panel = self._panel(ctrl_row, "모드 제어", MUTED)
        mode_panel.grid(row=0, column=0, sticky="nsew")
        mode_inner = tk.Frame(mode_panel, bg=SURFACE)
        mode_inner.pack(padx=8, pady=8, fill="both", expand=True)
        self.mode_btns = {}
        btn_specs = [
        ...
        for i, (label, cmd, req_mode, confirm) in enumerate(btn_specs):
            btn = tk.Button(mode_inner, text=label, font=("Arial", 10, "bold"), bg=SURFACE2,
                            fg=MUTED, bd=0, highlightthickness=1, highlightbackground=BORDER2,
                            padx=6, pady=10, cursor="hand2", state="disabled",
        ...
        mode_inner.rowconfigure((0, 1), weight=1)
    ...
    def _build_timeline_panel(self, parent):
        ...
        for i, (lbl, clr) in enumerate([("발사시각", CYAN), ("사출시각", AMBER), ("착륙시각", GREEN)]):
            ...
            tk.Label(card, text=lbl.upper(), font=("Arial", 8, "bold"), bg=SURFACE2, fg=MUTED,
                     anchor="w").pack(fill="x", padx=6, pady=(3, 0))
            v = tk.Label(card, text="--:--:--", font=("Courier", 11, "bold"), bg=SURFACE2, fg=clr, anchor="w")
            v.pack(fill="x", padx=6, pady=(0, 0))
            sub = tk.Label(card, text="", font=("Arial", 8), bg=SURFACE2, fg=MUTED, anchor="w")
    ...
    def _build_event_log(self, parent, row=4, col=0):
        ...
        self.event_log_text = tk.Text(
            log_inner, bg=SURFACE2, fg=TEXT, font=("Courier", 10), bd=0,
    ...
    def _build_emer_row(self, parent):
        ...
        tk.Label(tl2_inner, text="경과시간 (발사 후 · 착륙 시 정지)", font=("Arial", 8, "bold"),
                 bg=SURFACE, fg=MUTED, anchor="w", wraplength=240, justify="left").pack(fill="x")
    ...
    def _build_emer_idle(self):
        ...
        self.emer_btn = tk.Button(
            self.emer_inner, text="⚠ 비상사출", font=("Courier", 16, "bold"), bg="#7a0000",
    ...
    def _emer_click(self):
        ...
        tk.Label(dlg, text="⚠ 비상사출을 정말 실행하시겠습니까?", font=("Courier", 11, "bold"),
    ...
    def _on_close(self):
        ...
        try:
            ...
            pass
        # 3D 창이 이 Tk 창 안에 자식 창으로 끼워져 있는 상태라, Tk를 먼저
        # destroy()하면 Windows가 자식 창을 강제로 같이 지워버려서 pywebview가
        # "정상적으로 닫혔다"는 신호를 못 받고 webview.start()가 멈춰있을 수 있음
        # — 그래서 3D 창을 먼저 정상 종료하고 나서 Tk 창을 닫음.
    ...
    def _tick(self):
        ...
        if self.link:
            while True:
                ...
                if pkt.flight_mode == 4 and pm != 4:
                    ...
                    self.landed_ref_ms = pkt.ms   # 서보 전원 유지시간(SERVO_CLOSE_HOLD_MS) 계산 기준

                self.last_pkt = pkt

                # 최고고도 running max — 상승 중엔 계속 갱신되고, 하강 시작하면
                # (현재고도 < 최고고도) 자연스럽게 더 이상 안 늘어나서 정점에 고정됨
                if self.peak_alt_m is not None and pkt.altitude_cm != ALT_INVALID:
                    self.peak_alt_m = max(self.peak_alt_m, pkt.altitude_cm / 100.0)
        ...
        if last:
            ...
            for name, ok in sensor_ok.items():
                f, inner, dot, lbl = self.sensor_frames[name]
            ...
            # 펌웨어(Rudder2026.ino loop())의 실제 MOSFET 로직과 동일하게 계산:
            # SAFE는 항상 OFF, ARMED/FLIGHT/DESCENT는 항상 ON, LANDED는 착륙 직후
            # SERVO_CLOSE_HOLD_MS(서보 닫힘 복귀 유지시간) 동안만 ON, 그 이후 OFF.
            # (이전엔 "모드가 0이 아니면 ON"으로 단순화해서 착륙 후에도 계속 ON으로
            #  잘못 표시되던 버그가 있었음)
            if cur_mode in (1, 2, 3):
                servo_on = True
            elif cur_mode == 4:
                servo_on = (self.landed_ref_ms is not None
                            and (last.ms - self.landed_ref_ms) < SERVO_CLOSE_HOLD_MS)
            else:
                servo_on = False
            sf, sinner, sdot, slbl = self.sensor_frames["서보"]
            sbg = GREEN_DIM if servo_on else SURFACE2
            sfg = GREEN if servo_on else MUTED
            sf.config(bg=sbg, highlightbackground=sfg)
            sinner.config(bg=sbg)
            sdot.config(bg=sbg, fg=sfg)
            slbl.config(text="서보 ON" if servo_on else "서보 OFF", bg=sbg, fg=sfg)

            self.cmdrx_lbl.config(text=str(last.cmd_rx_count))
            tilt_xy_deg = last.tilt_xy / 100.0
            tilt_xz_deg = last.tilt_xz / 100.0
            if _webview_window is not None:
                try:
                    _webview_window.evaluate_js(f"setTilt({tilt_xy_deg}, {tilt_xz_deg})")
                except Exception:
                    pass
        ...
        self.reset_btn.config(
            ...
            highlightbackground=CYAN if reset_enabled else BORDER2)
        self._sync_emer_button_state()
        ...
        # launch_ref_ms가 없어지면(리셋 등) 갱신을 멈추기만 하고 이전 값이 그대로
        # 남아있던 버그 — else에서 자리표시자로 되돌림
        ...
        elif self.launch_ref_ms is None:
    ...
    def _write_log(self, pkt: Pkt):
        self.log_writer.writerow({
            ...
            'tilt_xy': pkt.tilt_xy / 100.0, 'tilt_xz': pkt.tilt_xz / 100.0,
        })
        self.log_file.flush()
...
def _run_tk_app():
    app = RocketGCS()
    app.mainloop()
    # Tkinter가 닫히면 3D 창도 같이 닫아서 프로세스 전체가 종료되게
    if _webview_window is not None:
        try:
            _webview_window.destroy()
        except Exception:
...
if __name__ == "__main__":
    if _HAS_WEBVIEW:
        try:
            html_path = pathlib.Path(__file__).parent / 'assets' / 'attitude3d.html'
            _webview_window = webview.create_window("로켓 자세 3D", html_path.as_uri(),
                                                      width=520, height=520)
            # pywebview는 반드시 메인 스레드에서 start() 해야 함 — Tkinter는
            # func으로 넘겨서 별도 스레드에서 돌림 (webview.start가 그렇게 실행해줌)
            webview.start(_run_tk_app)
        except Exception as e:
            print(f"[3D] pywebview 실행 실패 — 3D 창 없이 계속 진행: {e}")
