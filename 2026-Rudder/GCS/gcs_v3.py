...
def _panel():
    ...
    if link:
        while True:
            ...
            st.session_state.alt_hist.append(
                pkt.altitude_cm / 100.0 if pkt.altitude_cm != -1 else None)
    ...
    with c_num:
        ...
        if last:
            ...
            alt = f"{last.altitude_cm / 100:.1f} m" if last.altitude_cm != -1 else "BMP 오류"
            n1.metric("고도", alt)
...
def _charts():
    ...
    st.plotly_chart(fig, use_container_width=True, key="v3_chart")
